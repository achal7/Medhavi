module Medhavi.Foundation.Validations
...
let requiredRes (field: string) (value: string) (code: string) =
    if String.IsNullOrWhiteSpace(value.Trim()) then
        Error(DomainError.validation(code, $"{field} is required"))
    else
        Ok()

let matchRes (left: 'T) (right: 'T) (message: string) (code: string) =
    if left = right then Ok() else Error(DomainError.validation(code, message))
