module Medhavi.Foundation.InMemRepository

open System.Collections.Concurrent
open Medhavi.Foundation.Contracts.Aggregate
let create<'Aggregate, 'Id, 'Event when 'Id: not null> () =
    ...
    { Get =
      ...
      GetWithVersion =
        fun id ->
            task {
                match store.TryGetValue id with
                | true, versioned -> return Ok(Some versioned.Aggregate, Some versioned.Version)
                | false, _ -> return Ok(None, None)
            }
      Save =
        fun (id, versionOpt, aggregate, events) ->
            task {
                ...
                return
                    lock lockObj (fun () ->
                        ...
                        | true, existing ->
                            ...
                            let newVersioned =
                                { Aggregate = aggregate
                                  Version = versionOpt |> Option.defaultValue (existing.Version + 1) }

                            store[id] <- newVersioned
                        | false, _ ->
                            let newVersioned = { Aggregate = aggregate; Version = versionOpt |> Option.defaultValue 1 }
                            store[id] <- newVersioned
