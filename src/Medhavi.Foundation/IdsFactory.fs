module Medhavi.Foundation.IdsFactory
...
let private hashParts (parts: string list) =
    ...
    bytes |> Array.take 12 |> Array.fold (fun acc b -> acc + b.ToString("x2")) ""
...
/// Deterministic ID from a prefix and an ordered list of string parts.
/// Uses the same SHA‑256 hashing as other DeterministicIds helpers.
/// Result format: "{prefix}-{shortHash}"
let generalDeterministicId (prefix: string) (parts: string list) =
    let hash = hashParts(prefix :: parts)
    $"{prefix}-{hash}"
...
let createDeterministicId (externalSystemId: ExternalSystemId) (aggregateType: string) : string =
    ...
    let hashString = BitConverter.ToString(hashBytes).Replace("-", "").ToLowerInvariant()
...
let createId (strategy: IdGenerationStrategy) (ctor: string -> 'id) (name: string) =
    generateId strategy name |> Result.bind(fun id -> Ok(ctor id))
