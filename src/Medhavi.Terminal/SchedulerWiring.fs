            ...
            fun skuId spId ->
                task {
                    ...
                    let matchOpt =
                        invs
                        |> List.tryFind (fun i ->
                            i.SkuId = SkuId.value skuId
                            && i.StockingPointId = StockingPointId.value spId)
            ...
            fun skuId spId ->
                task {
                    ...
                    let matchOpt =
                        targets
                        |> List.tryFind (fun t ->
                            t.SkuId = SkuId.value skuId
                            && t.StockingPointId = StockingPointId.value spId)
        ...
        let supplierQuery: SupplyGenerationStep.SupplierQuery =
            fun skuId spId ->
                task {
                    ...
                    let offerOpt =
                        offers
                        |> List.tryFind (fun o -> o.SkuId = SkuId.value skuId && o.IsActive)

                    return
                        offerOpt
                        |> Option.map (fun o -> SupplierId.create o.SupplierId |> Result.get)
                }
            ...
            fun skuId spId ->
                task {
                    ...
                    let matchOpt =
                        ...
                        |> List.tryFind (fun r ->
                            r.Status
                            && (r.Applicability.StockingPointId = None || r.Applicability.StockingPointId = Some (StockingPointId.value spId))
                    ...
                    return
                        matchOpt
                        |> Option.map (fun r -> RoutingId.create r.Id |> Result.get)
                }
        ...
        let transferSourceQuery: SupplyGenerationStep.TransferSourceQuery =
            fun skuId spId ->
                task {
                    ...
                    return
                        matchOpt
                        |> Option.map (fun l -> StockingPointId.create l.Origin |> Result.get)
                }
        ...
        let capacityPromiseQuery: CapacityPromiseQuery =
            fun rgId desiredBucket neededDuration tentativeLoad ->
                task {
                    ...
                    let targetDateOfBucket (bIdx: BucketIndex) =
                        startDate.AddDays(float bIdx)
                    ...
                    let rec findFeasible bIdx =
                        if bIdx > horizonEndBucket then
                            { EarliestFeasibleBucket = bIdx; IsFeasible = false }
                        else
                            ...
                            if neededMins <= remaining then
                                { EarliestFeasibleBucket = bIdx; IsFeasible = (bIdx <= desiredBucket) }
                            else
                                findFeasible (bIdx + 1)
            ...
            fun skuId spId routingIdOpt qty ->
                task {
                    ...
                    let matchOpt =
                        ...
                        | Some rId ->
                            routings
                            |> List.tryFind (fun r -> r.Id = RoutingId.value rId && r.Status)
                        | None ->
                            routings
                            |> List.filter (fun r ->
                                r.Status
        ...
        let reservationCreator: PostprocessStep.ReservationCreator =
            fun skuId spId qty reqDate ->
                async {
                    ...
                    let! res =
                        supplyContext.Commands.MaterialReservation.CreateTentative req
                        |> Async.AwaitTask
        ...
        let createSupplyOrders: CreateSupplyOrders =
            fun runId proposals ->
                async {
                    ...
                    let workOrders =
                        proposals
                        |> List.filter (fun p -> p.ProposalType = PlannedWorkOrder)
                    ...
                    for p in workOrders do
                        ...
                        let selectedRoutingOpt =
                            ...
                            | None ->
                                ...
                                let preferredOpt =
                                    ...
                                    |> List.tryHead
                                preferredOpt
                                |> Option.orElse (loadProfiles |> List.sortBy (fun r -> r.PreferencePriority) |> List.tryHead)

                        match selectedRoutingOpt with
                        | None ->
                            printColor ConsoleColor.Yellow (sprintf "  [WARNING] No routing found for planned work order %s (Sku: %s)" 
                        ...
                        | Some routing ->
                            ...
                            match FiniteCapacityScheduler.scheduleWorkOrder req routing activeResources bucketsState DateTimeOffset.UtcNow allocations with
                            | Error err ->
                                printColor ConsoleColor.Red (sprintf "  [ERROR] Scheduling failed for %s: %A" (SupplyProposalId.value p.Id) err)
                            | Ok (capResult, nextAllocations) ->
                                ...
                                if not capResult.Violations.IsEmpty then
                                    ...
                                    for v in capResult.Violations do
                                        ...
                                        | DueDateMiss(woId, start, now) ->
                                            printColor ConsoleColor.Red (sprintf "  [VIOLATION] Due Date Miss: start time %s is in the past (now is %s)" 
                                        ...
                                        | CapacityOverload(resId, bucketIdOpt, date, reqMins, availMins) ->
                                            ...
                                            printColor ConsoleColor.Red (sprintf "  [VIOLATION] Capacity Overload on resource %s (Bucket: %s, Date: %s): requested %.2f mins, but only %.2f mins remaining (overloaded by %.2f mins)"
                                                resId bIdStr (date.ToString("yyyy-MM-dd")) reqMins availMins overload)
                                        | ResourceUnavailable resId ->
                                            printColor ConsoleColor.Red (sprintf "  [VIOLATION] Resource %s is unavailable" resId)
                                        | CalendarViolation(calId, date) ->
                                            printColor ConsoleColor.Red (sprintf "  [VIOLATION] Calendar %s has a violation on date %s" calId (date.ToString("yyyy-MM-dd")))
                                printColor ConsoleColor.Cyan "=========================================\n"
                                ...
                                for op in prodOrder.Operations do
                                    ...
                                    let opWindow = { DateRange.Start = op.Window.Start; End = Some op.Window.End }
                                    let scheduleOpCmd =
                                        { Id = opId
                                ...
                                for resv in capResult.Reservations do
                                    ...
                                    let! _ =
                                        ...
                                        | None ->
                                            async {
                                                ...
                                                | Ok win ->
                                                    let! _ =
                                                        capacityContext.Capacity.DefineBucket(resv.ResourceId, win, DurationMinutes.create 480.0m |> Result.get)
                                                        |> Async.AwaitTask
                                                    ()
                                            }
                                    ...
                                    let! resvRes = capacityContext.CapacityReservation.Create reservationCmd |> Async.AwaitTask
                                    ()
                    ...
                    let! res =
                        supplyContext.Commands.SupplyOrder.CreateBulk reqs
                        |> Async.AwaitTask

                    match res with
                    | Ok _ -> 
                        printColor ConsoleColor.Green (sprintf "Finite scheduler run complete: %d production orders firmed, %d planning violations registered." 
