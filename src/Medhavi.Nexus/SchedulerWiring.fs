                ...
                async {
                    ...
                    for p in workOrders do
                        ...
                        | Some routing ->
                            ...
                            | Error err ->
                                printfn "  [ERROR] Scheduling failed for %s: %A" (SupplyProposalId.value p.Id) err
                                ...
                                if not capResult.Violations.IsEmpty then
                                    ...
                                    for v in capResult.Violations do
                                        ...
                                        | CapacityOverload(resId, bucketIdOpt, date, reqMins, availMins) ->
                                            ...
                                            printfn "  [VIOLATION] Capacity Overload on resource %s (Bucket: %s, Date: %s): requested %.2f mins, but only %.2f mins remaining (overloaded by %.2f mins)"
                                                resId bIdStr (date.ToString("yyyy-MM-dd")) reqMins availMins overload
                                        | ResourceUnavailable resId ->
                                            printfn "  [VIOLATION] Resource %s is unavailable" resId
                                        | CalendarViolation(calId, date) ->
                                            printfn "  [VIOLATION] Calendar %s has a violation on date %s" calId (date.ToString("yyyy-MM-dd"))
                                printfn "=========================================\n"
