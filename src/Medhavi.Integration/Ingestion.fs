...
module CsvHelper =
    type CsvRow = 
        ...
        member this.Get(columnName: string) : string option =
            let idx = Array.tryFindIndex (fun (h: string) -> h.Trim().Equals(columnName, StringComparison.OrdinalIgnoreCase)) this.Headers
            match idx with
        ...
        member this.GetDecimal(columnName: string) : decimal option =
            this.Get columnName |> Option.bind (fun v -> 
    ...
    let splitCsvLine (line: string) : string[] =
        let matches = System.Text.RegularExpressions.Regex.Matches(line, "(?<=^|,)(\"(?:[^\"]|\"\")*\"|[^,]*)")
    ...
    let parseCsv (csvText: string) : CsvRow[] =
        ...
        if rawLines.Length <= 1 then
            ...
            let headers = rawLines.[0].Split([| ',' |]) |> Array.map (fun s -> s.Trim().Trim('"'))
            rawLines.[1..]
            |> Array.filter (fun line -> not (String.IsNullOrWhiteSpace(line)))
            |> Array.map (fun line -> { Headers = headers; Values = splitCsvLine line })
...
module InboundAdapter =
    ...
    let private parseJsonList<'T> (json: string) : Result<'T list, string> =
    ...
    let parseUomCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let factor = row.GetDecimal "ToBaseFactor" |> Option.defaultValue 1.0m
            let created = row.GetDateTimeOffset "Created" |> Option.defaultValue DateTimeOffset.UtcNow
            { Id = id; Code = code; Name = name; IsBase = isBase; ToBaseFactor = factor; Created = created }
    ...
    let parseUnitConversionCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let created = row.GetDateTimeOffset "Created" |> Option.defaultValue DateTimeOffset.UtcNow
            { SourceUom = src; TargetUom = target; ConversionFactor = factor; Created = created }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseProductCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            { SkuId = id; Name = name; UoM = uom; IsActive = active }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseRoutingCsv csv =
        ...
        |> List.map (fun (skuId, items) -> 
            { SkuId = skuId; Steps = items |> List.map snd })
        |> Ok
    ...
    let parseSupplierCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let name = row.Get "Name" |> Option.defaultValue ""
            let active = row.GetBool "IsActive" |> Option.defaultValue true
            { SupplierId = id; Name = name; IsActive = active }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseTransportLegCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let lt = row.GetFloat "LeadTimeMinutes" |> Option.defaultValue 0.0
            let cap = row.GetDecimal "Capacity"
            let capUnit = row.Get "CapacityUnit"
            let cutoff = row.GetFloat "CutoffMinutes"
            let constraints = 
                match row.Get "Constraints" with
                | None -> []
                | Some s -> s.Split([| '|'; ';' |], StringSplitOptions.RemoveEmptyEntries) |> Array.toList |> List.map (fun x -> x.Trim())
            let rel = row.GetFloat "Reliability"
            ...
            let created = row.GetDateTimeOffset "Created" |> Option.defaultValue DateTimeOffset.UtcNow
            { Id = id; Origin = origin; Destination = dest; Mode = mode; Schedule = schedule
              LeadTimeMinutes = lt; Capacity = cap; CapacityUnit = capUnit; CutoffMinutes = cutoff
              Constraints = constraints; Reliability = rel; CO2PerUnit = co2
              EffectiveStart = start; EffectiveEnd = end'; Created = created }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseCustomerOrderCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let dt = row.GetDateTimeOffset "RequestedDateUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            { OrderId = orderId; SkuId = skuId; NodeId = nodeId; Quantity = qty; RequestedDateUtc = dt }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseForecastCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let nodeId = row.Get "NodeId" |> Option.defaultValue ""
            let qty = row.GetDecimal "Quantity" |> Option.defaultValue 0.0m
            let dt = row.GetDateTimeOffset "ForecastDateUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            { ForecastId = fcId; SkuId = skuId; NodeId = nodeId; Quantity = qty; ForecastDateUtc = dt }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseInventoryPositionCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let qty = row.GetDecimal "Quantity" |> Option.defaultValue 0.0m
            let dt = row.GetDateTimeOffset "AsOfUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            { ProductId = prod; StockingPointId = sp; Quantity = qty; AsOfUtc = dt }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseSupplyOrderStatusCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let sp = row.Get "StockingPointId" |> Option.defaultValue ""
            let qty = row.GetDecimal "Quantity" |> Option.defaultValue 0.0m
            let dt = row.GetDateTimeOffset "ExpectedDeliveryUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            let status = row.Get "Status" |> Option.defaultValue ""
            { SupplyOrderId = id; ProductId = prod; StockingPointId = sp; Quantity = qty; ExpectedDeliveryUtc = dt; Status = status }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseResourceCalendarCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            let resId = row.Get "ResourceId" |> Option.defaultValue ""
            let start = row.GetDateTimeOffset "StartUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            let end' = row.GetDateTimeOffset "EndUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            let factor = row.GetFloat "CapacityFactor" |> Option.defaultValue 1.0
            let reason = row.Get "Reason"
            { ResourceId = resId; StartUtc = start; EndUtc = end'; CapacityFactor = factor; Reason = reason }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseWorkOrderCompletedCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let qty = row.GetDecimal "QuantityCompleted" |> Option.defaultValue 0.0m
            let dt = row.GetDateTimeOffset "CompletedAtUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            { WorkOrderId = id; RoutingId = routing; QuantityCompleted = qty; CompletedAtUtc = dt }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseMaterialReceivedCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let prod = row.Get "ProductId" |> Option.defaultValue ""
            let sp = row.Get "StockingPointId" |> Option.defaultValue ""
            let qty = row.GetDecimal "QuantityReceived" |> Option.defaultValue 0.0m
            let dt = row.GetDateTimeOffset "ReceivedAtUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            { ReceiptId = id; ProductId = prod; StockingPointId = sp; QuantityReceived = qty; ReceivedAtUtc = dt }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseResourceDowntimeCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            { ResourceId = resId; StartUtc = start; EndUtc = end'; Reason = reason }
        rows |> Array.toList |> List.map parseRow |> Ok
    ...
    let parseTransportDelayCsv csv =
        ...
        let parseRow (row: CsvHelper.CsvRow) =
            ...
            let lt = row.GetFloat "EstimatedDelayMinutes" |> Option.defaultValue 0.0
            let dt = row.GetDateTimeOffset "NewArrivalUtc" |> Option.defaultValue DateTimeOffset.UtcNow
            let reason = row.Get "Reason" |> Option.defaultValue ""
            { TransportLegId = id; EstimatedDelayMinutes = lt; NewArrivalUtc = dt; Reason = reason }
        rows |> Array.toList |> List.map parseRow |> Ok
...
module MasterDataValidator =
    ...
    let validate (payload: MasterDataPayload) : Validation<MasterDataPayload, string> =
        ...
        let productIds = products |> List.map (fun p -> p.SkuId) |> Set.ofList
        let stockingPointIds = stockingPoints |> List.map (fun s -> s.StockingPointId) |> Set.ofList
        let resourceIds = resources |> List.map (fun r -> r.ResourceId) |> Set.ofList
        ...
        let validateUniqueProductIds =
            ...
            else Invalid [ sprintf "Duplicate Product IDs found: %A" duplicates ]
        ...
        let validateUniqueStockingPointIds =
            let duplicates = 
                ...
                |> List.map fst
            if List.isEmpty duplicates then Valid ()
            else Invalid [ sprintf "Duplicate Stocking Point IDs found: %A" duplicates ]
        ...
        let validateBomReferences =
            let invalidBoms =
                boms
                |> List.filter (fun b -> 
                    not (productIds.Contains(b.ParentSkuId)) || not (productIds.Contains(b.ComponentSkuId)))
            if List.isEmpty invalidBoms then Valid ()
            else 
                let details = invalidBoms |> List.map (fun b -> sprintf "Parent: %s, Component: %s" b.ParentSkuId b.ComponentSkuId)
        ...
        let validateResourceReferences =
            ...
            if List.isEmpty invalidResources then Valid ()
            else
                let details = invalidResources |> List.map (fun r -> sprintf "Resource: %s, Missing Node: %s" r.ResourceId r.NodeId)
        ...
        let validateRoutingReferences =
            ...
            | _, false -> 
                Invalid [ sprintf "Routing steps refer to missing Resource IDs: %A" invalidSteps ]
