...
module IntegrationService =
    ...
    let getCsvPath fileName =
        System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "csv", fileName)
    ...
    let ingestAndPublishMasterData (store: EnvelopeStoreOps) : Task<LoadResult> =
        task {
            try
                ...
                | Valid _ ->
                    // 1. SKUs
                    let skus = SkuAdapter.parse productsCsv |> Result.defaultWith (fun e -> failwith e)
                    // 2. BOMs
                    let bomsList = BomAdapter.parse bomsCsv |> Result.defaultWith (fun e -> failwith e)
                    // 3. Plants & Stocking Points
                    let sps, nodes = StockingPointAdapter.parse stockingPointsCsv |> Result.defaultWith (fun e -> failwith e)
                    // 4. Routings
                    let routingsList = RoutingAdapter.parse routingsCsv resourcesCsv boms |> Result.defaultWith (fun e -> failwith e)
                    // 5. Transport Legs
                    let legs = TransportLegAdapter.parse transportLegsCsv |> Result.defaultWith (fun e -> failwith e)

                    // 6. UOMs
                    let allUoms = UomAdapter.parse productsCsv transportLegsCsv |> Result.defaultWith (fun e -> failwith e)

                    // 7. Plants
                    let plantsList = PlantAdapter.parse stockingPointsCsv |> Result.defaultWith (fun e -> failwith e)

                    // 8. Unit Conversions
                    let conversionList = UnitConversionAdapter.parse unitConversionsCsv |> Result.defaultWith (fun e -> failwith e)
                    ...
                    | Error err -> 
                        return IngestionError (sprintf "Serialization failed: %A" err)
                    | Ok envelope ->
                        ...
                        | Error err -> 
                            return IngestionError (sprintf "Failed to write to EnvelopeStore: %A" err)
                        | Ok _ ->
                            return Success (envelope.EventId, correlationId)
            with ex ->
