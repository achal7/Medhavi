    ...
    let tests =
        testList
            ...
            [ testCase "should parse UnitConversion CSV completely" (fun () ->
                  let csv =
                      "SourceUom,TargetUom,ConversionFactor,Created\nKG,G,1000.0,2026-05-27T00:00:00Z"
              ...
              testCase "should parse Resource Groups CSV completely" (fun () ->
                  let csv =
                      "ResourceGroupId,PlantId,Name,Description,DefaultCalendarId,IsActive\nRG-1,PLANT-1,Group 1,Desc,CAL-1,true"
              ...
              testCase "should parse Standard Resources CSV completely" (fun () ->
                  let csv =
                      "StandardResourceId,ResourceGroupId,Name,Description,DefaultEfficiency,DefaultCostRateAmount,DefaultCostRateCurrency,IsActive\nSR-1,RG-1,Standard 1,Desc,0.85,150.0,USD,true"
              ...
              testCase "should parse Physical Resources CSV completely" (fun () ->
                  let csv =
                      "PhysicalResourceId,StandardResourceId,Name,SerialNumber,Location,EfficiencyOverride,CostRateOverrideAmount,CostRateOverrideCurrency,CalendarId,IsActive\nPR-1,SR-1,Physical 1,SN123,Room A,0.90,120.0,USD,CAL-2,true"
              ...
              testCase "should parse Routings and Steps from CSV completely (grouping steps)" (fun () ->
                  let routingCsv =
                      "RoutingId,Name,RoutingType,SkuId,StockingPointId,EffectiveStart,PreferencePriority,IsPreferred,CostPolicyType\nROUTING-SKU-1,Routing for SKU-1,Work,SKU-1,SP-FACTORY,2026-01-01T00:00:00Z,1,true,NoRoutingCost"

                  let stepsCsv =
                      "RoutingId,StepId,Sequence,OperationCode,Name,ResourceRequirementId,ResourceKind,ResourceLoadBasis,ResourceRequiredUnits,ResourceSelectionRule,OptionId,ResourceGroupId,ResourceId,SetupTimeMinutes,RunTimePerBaseQuantityMinutes\nROUTING-SKU-1,STEP-SKU-1-10,10,OP-10,Step 10,REQ-10,WorkCenter,PerUnit,1.0,AnyAllowed,OPT-10,RG-1,RES-1,90.0,15.0\nROUTING-SKU-1,STEP-SKU-1-20,20,OP-20,Step 20,REQ-20,WorkCenter,PerUnit,1.0,AnyAllowed,OPT-20,RG-2,RES-2,30.0,6.0"
              ...
              testCase "should parse TransportLegs CSV completely (handling constraints splitting)" (fun () ->
                  let csv =
                      "Id,Origin,Destination,Mode,Schedule,LeadTimeMinutes,Capacity,CapacityUnit,CutoffMinutes,Constraints,Reliability,CO2PerUnit,EffectiveStart,EffectiveEnd,Created\nLEG-1,SP-1,SP-2,Road,Daily,180.0,500.0,PCS,60.0,\"Hazmat|Fragile\",0.95,0.05,2026-05-27T00:00:00Z,,2026-05-27T00:00:00Z"
              ...
              testCase "should parse InventoryTargets CSV completely" (fun () ->
                  let csv =
                      "SkuId,StockingPointId,SafetyStockQty,MinQty,MaxQty,TargetServiceLevel,CoverDays,IsActive\nSKU-BIKE,SP-WAREHOUSE,10.0,5.0,50.0,0.95,5.0,true"
              ...
              testCase "should parse SupplierOffers CSV completely" (fun () ->
                  let csv =
                      "Id,SupplierId,SkuId,StockingPointId,Moq,LotSize,LeadTimeP50Minutes,LeadTimeP95Minutes,Reliability,Incoterm\nOFFER-1,SUP-1,SKU-1,SP-1,50.0,10.0,1440.0,2880.0,0.95,DDP"
              ...
              testCase "should enforce MaterialReservation aggregate lifecycle transitions" (fun () ->
                  ...
                  let cmd =
                      { Id = "RES-XYZ"
                        ...
                        SkuId =
                          SkuId.create "SKU-BIKE"
                          |> function
                              | Ok x -> x
                              | Error _ -> failwith "invalid SkuId"
                        StockingPointId =
                          StockingPointId.create "SP-WAREHOUSE"
                          |> function
                              | Ok x -> x
                              | Error _ -> failwith "invalid StockingPointId"
                        Quantity = 20.0m
                  ...
                  | Ok decision ->
                      ...
                      | Ok confirmDec ->
                          ...
                          let decReduce =
                              decide (Reduce { Id = "RES-XYZ"; NewQuantity = 12.0m }) confirmedState
                      ...
                      | Ok expireDec -> test <@ expireDec.NewState.State = "Expired" @>)
              ...
              testCase "should calculate date-wise ATP projections with active reservations correctly" (fun () ->
                  ...
                  | Ok list ->
                      ...
                      let day1Val =
                          list
                          |> List.find (fun (d, _) -> d.Date = startDate.Date)
                          |> snd
                      ...
                      let day11Val =
                          list
                          |> List.find (fun (d, _) -> d.Date = day11Date)
                          |> snd
                      ...
                      let day21Val =
                          list
                          |> List.find (fun (d, _) -> d.Date = day21Date)
                          |> snd
