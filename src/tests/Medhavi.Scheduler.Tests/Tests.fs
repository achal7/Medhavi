...
open Medhavi.Planning.Mrp.Domain.Algorithms.BomExplosion
open Medhavi.Planning.Mrp.Pipeline
        ...
        testList
            ...
            [ testCase "should calculate correct shortage when demand exceeds stock (legacy)" (fun () ->
              ...
              testCase "BOM explosion should catch cyclic relationships and raise error" (fun () ->
                  ...
                  let mockBomLookup sku _ =
                      if SkuId.value sku = "sku-a" then
                          Some { BomId = "bom-a"
                                 ParentSkuId = skuA
                                 Components = [ { ComponentSkuId = skuB; QuantityPer = Quantity.create 1m |> getOk; UnitOfMeasureId = UomId.create "pc" |> getOk; Sequence = 1; IsPhantom = false } ]
                                 IsActive = true }
                      elif SkuId.value sku = "sku-b" then
                          Some { BomId = "bom-b"
                                 ParentSkuId = skuB
                                 Components = [ { ComponentSkuId = skuA; QuantityPer = Quantity.create 1m |> getOk; UnitOfMeasureId = UomId.create "pc" |> getOk; Sequence = 1; IsPhantom = false } ]
                                 IsActive = true }
                      else None
              ...
              testCase "Lot sizing should calculate EOQ correctly" (fun () ->
                  ...
                  let eoqQty = LotSizing.eoq 1000m 20m 4m
                  test <@ Quantity.value eoqQty = 100m @>)

              // 4. Capacity CTP checks & alternate routing fallback
              testCaseAsync "Capacity CTP check should fall back onto alternate routing on overload" (async {
                  ...
                  let mockAlternateRoutingsQuery _ _ =
                      async { return [ primaryRouting; altRouting ] }
                  ...
                  let policy =
                      { MrpPolicy.defaults with
                          CapacityPolicy = { Finite = true; SafetyBuffer = None; ReliabilityFactor = None; MaxAlternateAttempts = 2; BottleneckProtection = None } }

                  let ctx = MrpContext.create (MrpRunId.create "run-1" |> getOk) targetDate targetDate spId policy

                  let step = CapacityCheckStep.createStep mockCapacityQuery mockAlternateRoutingsQuery
                  let! result = step [proposal] ctx
                  match result with
