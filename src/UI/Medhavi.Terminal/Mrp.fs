...
let runBaselineMrp (deps: MrpDependencies) (demandContext: Medhavi.Demand.DemandContext) printer =
    task {
    ...
    task {
        ...
        | ResourceDowntimes resourceDowntimes ->
            for payload in resourceDowntimes do
                ...
                | None -> printer.PrintLine Cyan "   - No baseline MRP run cached. Skipping heuristic reactive repair."
                | Some baseline ->
                    printer.PrintLine Cyan "   - Evaluating blast radius and triggering reactive repair..."
                    ...
                    | Error err ->
                        printer.PrintLine PrinterColor.Red (sprintf "   [ ERR ] Reactive repair failed: %A" err)
                    | Ok newRun ->
                        let delta = Replan.PlanDeltaCalculator.calculate baseline newRun
                        printer.PrintLine PrinterColor.Green "   [ OK ] Reactive repair complete."
                        ...
                        | Ok _ ->
                            printer.PrintLine
                                PrinterColor.Green
                                "     - Repaired plan successfully persisted to database."
                        | Error err ->
                            printer.PrintLine
                                PrinterColor.Red
                                (sprintf "     - Failed to persist repaired proposals: %s" err)
        ...
        | TransportDelays transportDelays ->
            ...
            ()
        | _ -> printer.PrintLine PrinterColor.Red (sprintf "INVALID REQUEST %A for MRP" req)
    }
