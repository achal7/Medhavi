module Medhavi.Scheduler.Mrp.Steps.CapacityCheckStep
...
type CapacityCheckQuery =
    StockingPointId
        ...
        -> Timestamp
        -> CapacityPolicy
        -> TaskResult<Timestamp, CapacityCheckError>

/// Query to look up alternate routings for a SKU at a stocking point.
type AlternateRoutingsQuery = SkuId -> StockingPointId -> Task<RoutingId list>
    ...
    fun proposals ctx ->
        task {
                    ...
                    task {
                        if ctx.Policy.CapacityPolicy.Finite then
                            ...
                            | Ok allocatedDate ->
                                if allocatedDate <= proposal.DueDate then
                                    ...
                                    return
                                        Ok
                                            { proposal with
                                                CapacityCheckedDate = Some allocatedDate }
                                else if
                                    // Delayed: Try alternate routings
                                    ctx.Policy.CapacityPolicy.MaxAlternateAttempts > 0
                                then
                                    ...
                                    | _ ->
                                        ...
                                        let! alternateResults = Task.WhenAll(alternateTasks)
                                        ...
                                        | _ ->
                                                ...
                                                |> List.sortBy (fun (_, date) ->
                                                    let delay =
                                                        (Timestamp.value date
                                                         - Timestamp.value proposal.DueDate)
                                                            .TotalSeconds
                                            ...
                                            if bestDate < allocatedDate then
                                                ...
                                                return
                                                    Ok
                                                        { proposal with
                                                            RoutingId = Some bestRouting
                                                            DueDate = bestDate
                                                            CapacityCheckedDate = Some bestDate }
                                            else
                            ...
                            return
                                Ok
                                    { proposal with
                                        CapacityCheckedDate = Some proposal.DueDate }
                    })
            let! checkedWorkOrders = Task.WhenAll(checkedWorkOrderTasks)
            ...
            let failedProposalsWithFallback =
                ...
                |> List.map (fun (p, _) ->
                    { p with
                        CapacityCheckedDate = Some p.DueDate })
            ...
            let updatedCtx =
                ...
                |> MrpContext.updateTelemetry (fun t ->
                    { t with
                        CapacityCheckDuration = duration })
                |> (fun c ->
                    failures
                    |> List.fold
                        (fun acc (p, err) ->
