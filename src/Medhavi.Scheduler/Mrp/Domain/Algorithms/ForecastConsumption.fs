...
/// Phase 9.5: Forecast consumption
/// FP Pattern: Pure functions, pattern matching, recursive folds
module Medhavi.Planning.Mrp.Domain.Algorithms.ForecastConsumption
...
let isWithinWindow (strategy: ForecastConsumptionStrategy) (windowSize: TimeSpan) (orderDate: DateTimeOffset) (forecastStart: DateTimeOffset) (forecastEnd: DateTimeOffset) : bool =
    let (wStart, wEnd) =
        match strategy with
        | BackwardConsumption ->
            (orderDate - windowSize, orderDate)
        | ForwardConsumption ->
            (orderDate, orderDate + windowSize)
        | BidirectionalConsumption ->
            (orderDate - windowSize, orderDate + windowSize)
...
let consumeForecasts
    ...
    if not policy.Enabled then
        ...
        let forecastsGrouped = forecasts |> List.groupBy (fun f -> f.SkuId, f.NodeId) |> Map.ofList
        let ordersGrouped = orders |> List.groupBy (fun o -> o.SkuId, o.NodeId) |> Map.ofList
        
        let allKeys = 
            Set.union 
                (forecastsGrouped |> Map.keys |> Set.ofSeq) 
                (ordersGrouped |> Map.keys |> Set.ofSeq)
                
        allKeys
        |> Seq.toList
        |> List.collect (fun (skuId, nodeId) ->
            let groupForecasts = forecastsGrouped |> Map.tryFind (skuId, nodeId) |> Option.defaultValue []
            let groupOrders = ordersGrouped |> Map.tryFind (skuId, nodeId) |> Option.defaultValue []
            ...
            let finalForecasts =
                ...
                ||> List.fold (fun currentForecasts order ->
                    ...
                    let candidates =
                        currentForecasts
                        |> List.filter (fun f ->
                            isWithinWindow policy.Strategy policy.ConsumptionWindow orderDate f.PeriodStart f.PeriodEnd)
                        |> List.sortBy (fun f -> abs (f.PeriodStart - orderDate).Ticks)
                        
                    let rec consume (remainingQty: decimal) (accForecasts: Forecast list) (candidatesLeft: Forecast list) =
                    ...
                    let nonCandidates =
                        currentForecasts
                        |> List.filter (fun f ->
                            not (isWithinWindow policy.Strategy policy.ConsumptionWindow orderDate f.PeriodStart f.PeriodEnd))
