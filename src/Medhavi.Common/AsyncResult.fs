...
module AsyncResult =
    ...
    let return' (value: 'T) : AsyncResult<'T, 'E> = async.Return(Ok value)
    ...
    let sequence (asyncResults: AsyncResult<'T, 'E> list) : AsyncResult<'T list, 'E> =
        ...
        List.foldBack (fun x acc -> consA <*> x <*> acc) asyncResults (return' [])
    ...
    let sequence' (asyncResults: AsyncResult<'T, 'E> list) : AsyncResult<unit, 'E> =
        let folder _ acc = bind (fun _ -> acc) (return'())
        List.foldBack folder asyncResults (return'())
...
type AsyncResultBuilder() =
    member _.Return(x) = AsyncResult.return' x
    member _.Bind(x, f) = AsyncResult.bind f x
    member _.ReturnFrom(x) = x
    member _.Zero() = AsyncResult.return'()
    member _.Delay(f) = f
    member _.Run(f) = f()
    ...
    // Control flow
    member _.IfThenElse(condition, ifBody, elseBody) = if condition then ifBody() else elseBody()
