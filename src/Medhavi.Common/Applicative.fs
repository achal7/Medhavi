...
module AsyncResultApplicative =
    let return' (x: 'T) = async.Return(Ok x)
...
module ApplicativeUtils =
    ...
    let liftA2 map apply f x y = apply (map f x) y

    /// Lift a three-parameter function to any applicative given its map and apply implementations
    let liftA3 map apply f x y z = apply (apply (map f x) y) z

    /// Sequence a list of applicative values given its return', map, and apply implementations
    let sequenceA return' map apply xs =
        let cons head tail = head :: tail
        let pureEmpty = return' []

        List.foldBack (fun x acc -> apply (map cons x) acc) xs pureEmpty

/// APPLICATIVE LAWS VERIFICATION
/// =============================

module ApplicativeLaws =
    /// Identity: return' id <*> v ≡ v
    let identityLaw (v: Option<'T>) =
        let pureId = OptionApplicative.return' id
        let left = OptionApplicative.apply pureId v
        left = v

    /// Homomorphism: return' f <*> return' x ≡ return' (f x)
    let homomorphismLaw (f: 'T -> 'U) (x: 'T) =
        let pureF = OptionApplicative.return' f
        let pureX = OptionApplicative.return' x
        let left = OptionApplicative.apply pureF pureX
        let right = OptionApplicative.return'(f x)
        left = right

    /// Interchange: u <*> return' y ≡ return' ($ y) <*> u
    let interchangeLaw (u: Option<'T -> 'U>) (y: 'T) =
        let pureY = OptionApplicative.return' y
        let left = OptionApplicative.apply u pureY
        let dollarY = fun f -> f y
        let pureDollarY = OptionApplicative.return' dollarY
