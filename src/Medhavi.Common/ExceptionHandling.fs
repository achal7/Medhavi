...
module CorrelationId =
    ...
    let toString (CorrelationId id) = id
...
type ExceptionContext =
    {   CorrelationId: CorrelationId
        ServiceName: string
        OperationName: string
        Logger: (string -> unit) // Simple logger function
...
let createErrorResponse (errorInfo: ErrorInfo) (errorCode: string) (severity: ErrorSeverity) =
    {   CorrelationId = CorrelationId.value errorInfo.CorrelationId
...
let logError (ctx: ExceptionContext) (errorInfo: ErrorInfo) (severity: ErrorSeverity) =
    ...
    let message =
        sprintf
            "[%s] Error in %s.%s [CorrelationId: %s]: %s"
            severityStr
            errorInfo.ServiceName
            errorInfo.OperationName
            (CorrelationId.value errorInfo.CorrelationId)
    ...
    task {
        ...
        with ex ->
            ...
            | Retry(maxRetries, delayMs) ->
                ...
                ctx.Logger(sprintf "Retry strategy not yet implemented for %s" ctx.OperationName)
                return Microsoft.FSharp.Core.Result.Error ex
            | Fallback(fallbackValue) ->
                ctx.Logger(sprintf "Using fallback value for %s" ctx.OperationName)
                return Ok(fallbackValue :?> 'T)
            | CircuitBreak(serviceName) ->
                ctx.Logger(sprintf "Circuit breaker triggered for service %s" serviceName)
                return Microsoft.FSharp.Core.Result.Error ex
            | LogAndContinue ->
                ctx.Logger(sprintf "Continuing after error in %s" ctx.OperationName)
                return Microsoft.FSharp.Core.Result.Error ex
            | FailFast ->
                ctx.Logger(sprintf "Failing fast due to critical error in %s" ctx.OperationName)
                return Microsoft.FSharp.Core.Result.Error ex
    }
...
let executeWithTimeout
    ...
    task {
        ...
        | :? OperationCanceledException ->
            let errorInfo =
                createErrorInfo ctx (Exception($"Operation timed out after {timeoutMs}ms")) contextData

            logError ctx errorInfo Error
            return Microsoft.FSharp.Core.Result.Error(Exception($"Operation timed out after {timeoutMs}ms"))
        | ex ->
            let errorInfo = createErrorInfo ctx ex contextData
            logError ctx errorInfo Error
            return Microsoft.FSharp.Core.Result.Error ex
...
type TaskResultBuilder() =
    member _.Bind(taskResult: Task<Result<'T, 'E>>, binder: 'T -> Task<Result<'U, 'E>>) =
        task {
            ...
            | Microsoft.FSharp.Core.Result.Error error -> return Microsoft.FSharp.Core.Result.Error error
        }
...
let exceptionToResult (operation: unit -> 'T) =
    try
        Ok(operation ())
    with ex ->
        Microsoft.FSharp.Core.Result.Error ex
