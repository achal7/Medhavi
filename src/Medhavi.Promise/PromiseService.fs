...
let calculateConfidence
    ...
    | Some rc, Some it ->
        ...
        | Some r1 -> Some(min r1 (float it.TotalReliability))
...
let checkTransportStep (trans: TransportProvider) : PromiseStep =
    fun ctx ->
        async {
            ...
            match res with
            | Ok itineraries ->
                let best = itineraries |> List.sortBy (fun it -> it.TotalLeadTimeMinutes) |> List.tryHead
                match best with
                | Some it ->
                ...
                | None ->
                    return Error ProviderError.NoTransportLeg
            | Error e -> return Error e
        }

// 5. Date Calculation Step
let calculateDateStep : PromiseStep =
    fun ctx ->
        async {
            ...
            // Fix inverted comments: Earliest is optimistic, Latest is conservative/committed
            let dateRange = { Earliest = committed; Committed = committed; Latest = committed }
...
let enrichCostConfidenceStep : PromiseStep =
    fun ctx ->
        async {
            let committed = ctx.PromiseDate |> Option.map (fun d -> d.Committed) |> Option.defaultValue ctx.Request.AsOfDate
            let cost = CostCalculation.calculateCost ctx.Policy (Quantity.value ctx.Line.Quantity) ctx.MaterialSnapshot None ctx.Itinerary committed ctx.Line.DueDate
            let conf = calculateConfidence ctx.RoutingChoice ctx.Itinerary
            
            // Bottleneck limiter selection
...
let processLine
    ...
    // Execute pipeline steps in monadic sequence
    async {
        ...
        | Error e -> return Error e
        | Ok c2 ->
        let! r3 = checkCapacityStep cap c2
        match r3 with
        | Error e -> return Error e
        | Ok c3 ->
        ...
        match r6 with
        | Error e ->
            // Roll back reservations if failure happens after creation
            return Error e
        | Ok c6 ->
...
let tryPromiseOrder
    ...
    async {
        if List.isEmpty req.Order.Lines then
            ...
            let! perLineResults = req.Order.Lines |> List.map runLine |> Async.Parallel
