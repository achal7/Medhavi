...
let initPromiseLineCtx (req: PromiseRequestCmd) (line: OrderLine) (policy: PromisePolicy) : PromiseLineCtx =
    { Request = req
      Line = line
      Policy = policy
      Routing = None
...
let createRejection domain code message =
    let limiter =
        { Domain = domain
          Code = code
          Message = message
    ...
    { Decision = PromiseDecisionStatus.Rejected limiter
      Routing = None
      Itinerary = None
      Material = None
      Cost = None
...
let earliestMaterialReady
    ...
    let reservationsToSubtract =
        if policy.EnableReservationSubtraction then
            snap.Reservations
        else
            0m
        ...
        |> List.fold
            (fun (accQty, ready) inbound ->
                ...
                | None ->
                    ...
                    if acc >= shortfall then
                        acc, Some(fst inbound)
                    else
                        acc, None)
            (0m, None)
        |> snd
...
let calculateConfidence (routingChoice: RoutingChoice option) (itinerary: Itinerary option) =
...
let bind
    (step: PromiseStep)
    (input: Async<Result<PromiseLineCtx, ProviderError>>)
...
let checkMaterialStep (mat: MaterialProvider) : PromiseStep =
    fun ctx ->
        async {
            ...
            | Ok snap ->
                let ready =
                    earliestMaterialReady ctx.Policy snap (Quantity.value ctx.Line.Quantity) ctx.Request.AsOfDate

                return
                    Ok
                        { ctx with
                            MaterialSnapshot = Some snap
                            MaterialReady = ready }
            | Error e -> return Error e
        }
...
let checkTransportStep (trans: TransportProvider) : PromiseStep =
    fun ctx ->
        async {
            let origin =
                ctx.Line.Origin
                |> Option.map (fun sp -> StockingPointId.value sp)
                |> Option.defaultValue (string ctx.Line.StockingPointId)

            let dest =
                ctx.Line.Destination
                |> Option.map (fun sp -> StockingPointId.value sp)
                |> Option.defaultValue (string ctx.Line.StockingPointId)
                ...
                let validItineraries =
                    ...
                    |> List.filter (fun it ->
                        ...
                        |> List.forall (fun hop ->
                            ...
                            hop.ArrivalDateOffset > 0m
                            && hop.LeadTimeMinutes > 0m))

                let best =
                    validItineraries
                    |> List.sortBy (fun it -> it.TotalLeadTimeMinutes)
                    |> List.tryHead
                ...
                | Some it ->
                    ...
                    return
                        Ok
                            { ctx with
                                Itinerary = Some it
                                TransportReady = Some arrival }
                | None -> return Error ProviderError.NoTransportCapacity
...
let checkSupplierOptionsStep (mat: MaterialProvider) : PromiseStep =
    fun ctx ->
        async {
            ...
            | Some _, Some ready when ready <= ctx.Request.AsOfDate ->
            ...
            | Some _, _ ->
                ...
                if ctx.Policy.CallSupplierOnShortfall then
                    ...
                    let! res =
                        mat.GetSupplierOptions(
                            ctx.Line.SkuId,
                            ctx.Line.StockingPointId,
                            requiredQty,
                            ctx.Request.AsOfDate
                        )
                    ...
                    return Ok { ctx with SupplierOption = None }
            | None, _ -> return Ok { ctx with SupplierOption = None }
        }
        ...
        async {
            ...
            if hasUnavailable then
                ...
                let confidencePenalty =
                    ...
                    | Some it ->
                        match it.TotalReliability with
...
let createReservationsStep (resv: ReservationProvider) : PromiseStep =
    fun ctx ->
        async {
            let committed =
                ctx.PromiseDate
                |> Option.map (fun d -> d.Committed)
                |> Option.defaultValue ctx.Request.AsOfDate
            ...
            let requests =
                [ { Scope = ReservationScope.Material
                  ...
                  { Scope = ReservationScope.Capacity
                    Reference =
                      ctx.CapacityResult
                      |> Option.bind (fun r -> r.BottleneckResourceId)
                      |> Option.defaultValue ""
                    SkuId = ctx.Line.SkuId
                    StockingPointId = ctx.Line.StockingPointId
                    Quantity = Quantity.value ctx.Line.Quantity
                  ...
                  { Scope = ReservationScope.Transport
                    Reference =
                      ctx.Itinerary
                      |> Option.map (fun it -> string it.Id)
                      |> Option.defaultValue ""
                    SkuId = ctx.Line.SkuId
                    StockingPointId = ctx.Line.StockingPointId
                    Quantity = Quantity.value ctx.Line.Quantity
        ...
        async {
            ...
            let cost =
                CostCalculation.calculateCost
                    ctx.Policy
                    (Quantity.value ctx.Line.Quantity)
                    ctx.MaterialSnapshot
                    ctx.SupplierOption
                    ctx.Itinerary
                    committed
                    ctx.Line.DueDate
...
let processLine
    ...
    let policy =
        PolicyHelpers.resolveFromTiers None req.CustomerTier req.SkuTier PolicyPresets.defaultPolicy
    ...
    async {
        ...
        | Ok ctx ->
            let decision =
                ...
                | None -> PromiseDecisionStatus.Accepted ctx.PromiseDate
                | Some limiter -> PromiseDecisionStatus.Rejected limiter
        ...
        | Error e ->
            let decision =
                createRejection PromiseLimiterDomain.System PromiseReasonCode.SearchTimeout (string e)
...
let tryPromiseOrder
    ...
    async {
        if List.isEmpty req.Order.Lines then
            let resp =
                createRejection PromiseLimiterDomain.Policy PromiseReasonCode.DataStale "No order lines to promise"
            ...
            if
                (policy.FullOrder && hasFailures)
                || successCount = 0
            then
                // Roll back all reservations
                let allResIds =
                    successCtxs
                    |> List.collect (fun c -> c.Reservations)

                let! _ = releaseReservations resv allResIds

                let message =
                    if policy.FullOrder then
                        "One or more order lines failed under FullOrder constraint."
                    else
                        "All order lines failed."

                let resp =
                    createRejection PromiseLimiterDomain.Policy PromiseReasonCode.FullOrderViolation message
                ...
                let dates =
                    successCtxs
                    |> List.choose (fun c -> c.PromiseDate)
                ...
                let fullDeliveryFailed =
                    policy.FullDelivery
                    && dates
                       |> List.exists (fun d -> d.Committed <> finalCommitted)
                ...
                if fullDeliveryFailed then
                    ...
                    let resp =
                        createRejection
                            PromiseLimiterDomain.Policy
                            PromiseReasonCode.FullDeliveryViolation
                            "Cannot promise full delivery - lines have different dates"

                    return Ok resp
                else
                    let finalDateRange =
                        { Earliest = finalCommitted
                          Committed = finalCommitted
                          Latest = finalCommitted }
                    ...
                    if costCapExceeded || riskCapExceeded then
                        ...
                        let code =
                            if costCapExceeded then
                                PromiseReasonCode.CostCapExceeded
                            else
                                PromiseReasonCode.RiskCapExceeded

                        let msg =
                            if costCapExceeded then
                                $"Total cost {totalCost} exceeds cost cap of {policy.CostCap.Value}"
                            else
                                $"Min confidence {minConfidence} is below risk cap of {policy.RiskCap.Value}"

                        let resp = createRejection PromiseLimiterDomain.Policy code msg
                        return Ok resp
                    else
                        // Success!
                        let routingChoice =
                            successCtxs
                            |> List.choose (fun c -> c.RoutingChoice)
                            |> List.tryHead

                        let itinerary =
                            successCtxs
                            |> List.choose (fun c -> c.Itinerary)
                            |> List.tryHead

                        let material =
                            successCtxs
                            |> List.choose (fun c -> c.MaterialSnapshot)
                            |> List.tryHead
                        ...
                        let resp =
                            { Decision = PromiseDecisionStatus.Accepted(Some finalDateRange)
