...
type ScenarioEvent =
    ...
    | ScenarioArchived of ScenarioId
...
module ScenarioAgg =
    ...
    let handle: Decide<Scenario, ScenarioCommand, ScenarioEvent> =
        fun command stateOpt ->
            ...
            | Rename newName, Some state when state.Status <> Archived ->
                let updated = { state with Name = newName }
                Ok { NewState = updated; Events = [ ScenarioRenamed(state.Id, newName) ] }

            | Rename _, Some _ -> errInvariant "Cannot rename an archived scenario"

            | MarkDirty, Some state when state.Status <> Archived ->
                let updated =
                    { state with
                        Version = Version.increment state.Version
                        IsDirty = true }
                Ok { NewState = updated; Events = [ ScenarioMarkedDirty state.Id ] }

            | MarkDirtyWith reason, Some state when state.Status <> Archived ->
                let updated =
                    { state with
                        Version = Version.increment state.Version
                        IsDirty = true
                        DirtyReason = Some reason }
                Ok { NewState = updated; Events = [ ScenarioDirtyReasonSet(state.Id, reason) ] }

            | (MarkDirty | MarkDirtyWith _), Some _ -> errInvariant "Cannot mark dirty an archived scenario"

            | MarkStructuralChange, Some state when state.Status <> Archived ->
                let updated =
                    { state with
                        StructuralChange = FullReplanRequired
                        IsDirty = true
                        Version = Version.increment state.Version }
                Ok { NewState = updated; Events = [ ScenarioStructuralChangeMarked state.Id ] }

            | MarkStructuralChange, Some _ -> errInvariant "Cannot mark structural change on an archived scenario"

            | ClearStructuralChange, Some state ->
                let updated = { state with StructuralChange = Unchanged }
                Ok { NewState = updated; Events = [ ScenarioStructuralChangeCleared state.Id ] }

            | Configure configId, Some state when state.Status <> Archived ->
                let updated =
                    { state with
                        ConfigurationId = Some configId
                        Version = Version.increment state.Version }
                Ok { NewState = updated; Events = [ ScenarioConfigured(state.Id, configId) ] }
                ...
                if not (Version.equals state.Version expectedVersion) then
                    ...
                    let updated =
                        { state with
                            ...
                            IsDirty = false
                            StructuralChange = Unchanged }
                    Ok { NewState = updated; Events = [ ScenarioPlanRefSet(state.Id, planRef) ] }

            | StartPlanning runId, Some state ->
                match state.Status with
                | PlanningRunning -> errConflict "Planning is already running"
                | Archived -> errInvariant "Cannot start planning on an archived scenario"
                | _ ->
                    ...
                    Ok { NewState = updated; Events = [ ScenarioPlanningStarted(state.Id, runId) ] }

            | StopPlanning, Some state ->
                match state.Status with
                | PlanningRunning ->
                    let updated = { state with Status = PlanningPaused }
                    Ok { NewState = updated; Events = [ ScenarioPlanningStopped state.Id ] }
            ...
            | RequestPlanning, Some state ->
                ...
                | Archived -> errInvariant "Cannot request planning on an archived scenario"
                | _ ->
                    let mode = ScenarioPolicy.determinePlanningMode state.StructuralChange state.DirtyReason
                    let updated =
                        { state with
                            Version = Version.increment state.Version
                            LastPlanningMode = Some mode }
                    Ok { NewState = updated; Events = [ ScenarioPlanningRequested(state.Id, mode) ] }

            | CompletePlanning(runId, planRef, inputVersions), Some state ->
                match state.Status with
                | PlanningRunning ->
                    match state.ActiveRunId with
                    | Some activeId when activeId <> runId -> errInvariant $"RunId mismatch: expected {activeId}, got {runId}"
                    | _ ->
                        let updated =
                            { state with
                                ...
                                DirtyReason = None
                                StructuralChange = Unchanged }
                        Ok { NewState = updated; Events = [ ScenarioPlanningCompleted(state.Id, runId, planRef, inputVersions) ] }
                | _ -> errConflict "Planning is not currently running"
                    ...
                    | _ ->
                        let updated =
                            { state with
                                ...
                                ActiveRunId = None }
                        Ok { NewState = updated; Events = [ ScenarioPlanningFailed(state.Id, runId, reason) ] }
                | _ -> errConflict "Planning is not currently running"
            ...
            | SubmitForApproval, Some state ->
                ...
                | WhatIf, PlanningComplete, Some _ ->
                    ...
                    Ok { NewState = updated; Events = [ ScenarioSubmittedForApproval state.Id ] }
                | WhatIf, PlanningComplete, None -> errInvariant "WhatIf scenario must have a completed plan before submission"
            ...
            | Approve, Some state ->
                ...
                | UnderReview ->
                    ...
                    Ok { NewState = updated; Events = [ ScenarioApproved state.Id ] }
            ...
            | Reject reason, Some state ->
                ...
                | UnderReview ->
                    ...
                    Ok { NewState = updated; Events = [ ScenarioRejected(state.Id, reason) ] }
                | s -> errInvariant (sprintf "Cannot reject from status %A" s)

            | Archive, Some state ->
                match state.Status with
                | Archived -> errInvariant "Scenario is already archived"
                | PlanningRunning -> errInvariant "Cannot archive a scenario while planning is running"
                | _ ->
                    let updated = { state with Status = Archived }
                    Ok { NewState = updated; Events = [ ScenarioArchived state.Id ] }
    ...
    let evolve (event: ScenarioEvent) (state: Scenario option) : Scenario option =
        ...
        | ScenarioMarkedDirty _, Some s ->
            Some { s with IsDirty = true; Version = Version.increment s.Version }

        | ScenarioDirtyReasonSet(_, reason), Some s ->
            Some { s with IsDirty = true; DirtyReason = Some reason; Version = Version.increment s.Version }

        | ScenarioStructuralChangeMarked _, Some s ->
            Some { s with StructuralChange = FullReplanRequired; IsDirty = true; Version = Version.increment s.Version }

        | ScenarioStructuralChangeCleared _, Some s ->
            Some { s with StructuralChange = Unchanged; Version = Version.increment s.Version }

        | ScenarioConfigured(_, configId), Some s ->
            Some { s with ConfigurationId = Some configId; Version = Version.increment s.Version }
        ...
        | ScenarioPlanningStarted(_, runId), Some s ->
            Some { s with Status = PlanningRunning; ActiveRunId = Some runId; Version = Version.increment s.Version }

        | ScenarioPlanningStopped _, Some s ->
            Some { s with Status = PlanningPaused; Version = Version.increment s.Version }

        | ScenarioPlanningRequested(_, mode), Some s ->
            Some { s with LastPlanningMode = Some mode; Version = Version.increment s.Version }
        ...
        | ScenarioPlanningFailed(_, _, _), Some s ->
            Some { s with Status = PlanningFailed; ActiveRunId = None; Version = Version.increment s.Version }

        | ScenarioSubmittedForApproval _, Some s ->
            Some { s with Status = UnderReview; Version = Version.increment s.Version }

        | ScenarioApproved _, Some s ->
            Some { s with Status = Approved; Version = Version.increment s.Version }

        | ScenarioRejected _, Some s ->
            Some { s with Status = Ready; Version = Version.increment s.Version }

        | ScenarioArchived _, Some s ->
            Some { s with Status = Archived; Version = Version.increment s.Version }
