...
module ObservationComposition =
    ...
    let createDevelopmentProvider () : ArchitecturalKnowledgeProvider =
        // let consoleRep: KnowledgeRepresentation =
        //     fun knowledge ->
        //         printfn $"[{knowledge.Timestamp:O}] {knowledge.Name}"
        //
        //         for kv in knowledge.Attributes do
        //             printfn $"  {kv.Key}: {kv.Value}"
        let logger = ConsoleLogger("Medhavi.Development") |> Logger.Create |> toKnowledgeRepresentation
        let telemetryRep = TelemetryPublisher.toKnowledgeRepresentation()

        [ logger; telemetryRep ]

    /// Create the production ArchitecturalKnowledgeProvider from an ILogger.
    let createProductionProvider (ilogger: ILogger) : ArchitecturalKnowledgeProvider =
