module Medhavi.Infrastructure.CircuitBreaker
...
module private Impl =
    ...
    let effectiveRecoveryTimeout (cfg: CircuitBreakerConfig) (consecutiveOpens: int) =
        if consecutiveOpens <= 1 then
            ...
            let scaled = TimeSpan.FromTicks(int64(float cfg.RecoveryTimeout.Ticks * multiplier))

            if scaled > cfg.MaxRecoveryTimeout then cfg.MaxRecoveryTimeout else scaled
    ...
    let handleSuccess
        ...
        | CircuitBreakerState.Closed ->
            pruneFailures cfg.MonitoringPeriod s.FailureTimestamps |> ignore
            // emit RequestSucceeded event
            cfg.OnEvent |> Option.iter(fun f -> f(RequestSucceeded whenHappened))

            logger |> Option.iter(fun l -> l.LogDebug("CircuitBreaker: request succeeded (Closed)."))
        ...
        | HalfOpen(successes, failures) ->
            ...
            if newSuccesses >= cfg.SuccessThreshold then
                ...
                cfg.OnEvent |> Option.iter(fun f -> f(Closed whenHappened))
        ...
        | Open(_) ->
            ...
            logger |> Option.iter(fun l -> l.LogDebug("CircuitBreaker: success seen while Open (ignored)."))
    ...
    let handleFailure
        ...
        | CircuitBreakerState.Closed ->
            if windowCount >= cfg.FailureThreshold then
                ...
                cfg.OnEvent |> Option.iter(fun f -> f(RequestFailed(whenHappened, ex.Message)))

                logger |> Option.iter(fun l -> l.LogWarning(ex, "CircuitBreaker: tripped to Open"))
                ...
                logger |> Option.iter(fun l -> l.LogDebug("CircuitBreaker: recorded failure (Closed)."))
        ...
        | HalfOpen(_, _) ->
            ...
            cfg.OnBreak |> Option.iter(fun cb -> cb(sprintf "Failure during HalfOpen: %s" ex.Message))

            cfg.OnEvent |> Option.iter(fun f -> f(Opened(whenHappened, "failure during half-open", newConsec)))

            cfg.OnEvent |> Option.iter(fun f -> f(RequestFailed(whenHappened, ex.Message)))

            logger |> Option.iter(fun l -> l.LogWarning(ex, "CircuitBreaker: failure in HalfOpen -> re-open"))
        ...
        | Open(openedAt) ->
            ...
            logger |> Option.iter(fun l -> l.LogDebug("CircuitBreaker: recorded failure while already Open."))
            ...
            let rec loop (state: State) =
                async {
                    ...
                    | Exec(op, tcs) ->
                        ...
                        let state =
                            ...
                            | Some s ->
                                ...
                                cfg.OnEvent |> Option.iter(fun f -> f(HalfOpened(nowUtc())))

                                logger |> Option.iter(fun l -> l.LogDebug("CircuitBreaker: Open -> HalfOpen"))
                        ...
                        | Open(_) ->
                            cfg.OnEvent |> Option.iter(fun f -> f(AvailabilityChanged(nowUtc(), false)))
                            ...
                            Task.Run(fun () ->
                                task.ContinueWith(fun (t: Task<'T>) ->
                                    if t.IsFaulted then
                                        ...
                                        inbox.Post(
                                            ReportFailure(nowUtc(), TaskCanceledException("task canceled"), Some tcs)
                                        )
                                    else
                                        try
                                            let res = t.Result
                    ...
                    | ReportSuccess(whenHappened, result, maybeTcs) ->
                        ...
                        maybeTcs |> Option.iter(fun tcs -> tcs.SetResult(Success result))
                    ...
                    | ReportFailure(whenHappened, ex, maybeTcs) ->
                        ...
                        maybeTcs |> Option.iter(fun tcs -> tcs.SetResult(ExecutionFailed(ex.Message)))
                    ...
                    | Reset ->
                        ...
                        cfg.OnEvent |> Option.iter(fun f -> f(ResetOccurred(nowUtc())))

                        logger |> Option.iter(fun l -> l.LogInformation("CircuitBreaker: Reset requested"))
                    ...
                    | IsAvailable(replyTcs) ->
                        ...
                        cfg.OnEvent |> Option.iter(fun f -> f(AvailabilityChanged(nowUtc(), available)))
...
let create (cfg: CircuitBreakerConfig) (logger: ILogger option) (isSuccess: ('T -> bool) option) : CircuitBreaker<'T> =
    ...
    let executeAsync (operation: unit -> Task<'T>) : Task<CircuitBreakerResult<'T>> =
        let tcs = TaskCompletionSource<CircuitBreakerResult<'T>>(TaskCreationOptions.RunContinuationsAsynchronously)
    ...
    let getStats () =
        let tcs = TaskCompletionSource<CircuitBreakerStats>(TaskCreationOptions.RunContinuationsAsynchronously)
