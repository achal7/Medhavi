...
// Follows decentralized error pattern - observability is module-scoped
...
/// Domain-level KPIs computed per planning scenario iteration
type PlanningKpis =
    { ScenarioId: string
      Timestamp: DateTimeOffset
      TotalCost: decimal
      ...
      ServiceLevel: float // Target % of on-time orders
      TenantId: string option }
...
type TelemetryMetric =
    ...
    | PlanningKpi of PlanningKpis
    | ErrorEvent of TelemetryErrorMetric

/// Telemetry module - Core operations
module Telemetry =
    ...
    let withCorrelation (correlationId: Guid) (causationId: Guid option) (event: TelemetryEvent) : TelemetryEvent =
        { event with
    ...
    let logWithCorrelation
        ...
        createEvent severity message properties
        |> withCorrelation correlationId None
        |> logEvent logger

    /// Log operation result as telemetry event
    let logResult (logger: Logger)
    ...
    let createLatency (name: string) (comp: string) (start: DateTimeOffset) (duration: TimeSpan) (isSuccess: bool) (errOpt: string option) (ctx: ExecutionContext) (metadata: Map<string, string>) : LatencyTelemetry =
        { OperationName = name
          Component = comp
          StartTime = start.ToUniversalTime()
          EndTime = (start + duration).ToUniversalTime()
    ...
    let createLimiterFrequency (limiterName: string) (comp: string) (current: float option) (limit: float option) (utilization: float option) (throttled: int64) (total: int64) (isActive: bool) (ctx: ExecutionContext) (metadata: Map<string, string>) : LimiterFrequencyTelemetry =
    ...
    /// Create a PlanningKpis record
    let createPlanningKpis (scenarioId: string) (totalCost: decimal) (latenessPenalty: decimal) (utilization: Map<string, float>) (serviceLevel: float) (ctx: ExecutionContext) : PlanningKpis =
...
module Performance =
    ...
    let measureAsync (operationName: string) (operation: Async<'T>) : Async<'T * PerformanceMeasurement> =
        async {
            let sw = Stopwatch.StartNew()
    ...
    let convertToTelemetryEvent (measurement: PerformanceMeasurement) : TelemetryEvent =
        let severity =
            if not measurement.Success then
                Warning
            elif measurement.Duration.TotalSeconds > 5.0 then
                Warning
            elif measurement.Duration.TotalSeconds > 1.0 then
                Information
            else
                Debug
    ...
    let convertToTelemetryEvent (health: ComponentHealth) : TelemetryEvent =
        ...
        let properties =
            ...
            |> fun m ->
                ...
                | Some rt ->
                    m
                    |> Map.add "ResponseTimeMs" (box rt.TotalMilliseconds)
                | None -> m
