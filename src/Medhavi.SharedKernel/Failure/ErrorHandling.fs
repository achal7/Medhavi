...
type DomainError =
    ...
    /// Generic data validation failure (before domain logic)
    ...
    /// A specific business rule has been violated.
    ...
    /// Conflicting state change (e.g. concurrency).
    ...
    let mapToApiError (error: ApplicationError) : Medhavi.Contracts.ApiError =
        ...
        | Domain e ->
            { Code = e.Code
              ...
              Message = e.Message }
        | Validation errors ->
            let msg = errors |> List.map(fun (f, m) -> $"{f}: {m}") |> String.concat "; "
