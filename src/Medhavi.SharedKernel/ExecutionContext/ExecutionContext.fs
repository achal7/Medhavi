...
module ExecutionContext =
    ...
    let withPrincipal (principal: string) (ctx: ExecutionContext) =
        { ctx with Principal = Some principal }

    let withTenantId (tenantId: string) (ctx: ExecutionContext) =
        { ctx with TenantId = Some tenantId }

    let withMessageId (messageId: string) (ctx: ExecutionContext) =
    ...
    let toMetadataMap (ctx: ExecutionContext) =
        ...
        let m = match ctx.CausationId with Some c -> m |> Map.add "causationId" (c |> CorrelationId.toString) | None -> m
        let m = match ctx.Principal with Some p -> m |> Map.add "principal" p | None -> m
        let m = match ctx.TenantId with Some t -> m |> Map.add "tenantId" t | None -> m
        let m = match ctx.MessageId with Some mid -> m |> Map.add "messageId" mid | None -> m
        m
